...
function Landmark() {
  ...
  const { landmark } = useParams(); // Extract location from URL params
  ...
  useEffect(() => {
    ...
    else if(landmark==="rooms"){
      setFilteredListings(listings);
    }else {
      const filtered = listings.filter((listing) =>
        listing.landmark.toLowerCase().includes(landmark.toLowerCase())
      );
      setFilteredListings(filtered);
    }
  }, [landmark, listings]); // Depend on `location` and `listings`
